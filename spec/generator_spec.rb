...
RSpec.describe "deterministic Ruby projection" do
  it "generates a loadable service with the NovaPay transformations" do
    ...
    Dir.mktmpdir("provider-compiler") do |directory|
      ...
      operation = { amount: 15_000.0, currency: "RUB", external_id: "op_abc123", idempotency_key: "key-1", recipient: { type: "sbp", phone: "79001234567", bank_code: "044525225" } }
      ...
      expect(service.check_conditions(operation.merge(amount: 1_500.50), "create")).to include("ok" => true)
      expect { service.build_create_request(operation.merge(amount: "not-a-money-value")) }.to raise_error(ArgumentError, /numeric/)
      expect(service.check_conditions(operation, "create")).to include("ok" => true)
      expect(service.check_conditions(operation.merge(recipient: { type: "sbp", phone: "79001234567" }), "create")).to include("ok" => false)
      expect(service.check_conditions(operation.merge(amount: 999.99), "create")).to include("ok" => false)
      ...
      expect(executed).to include("ok" => true, "provider_operation_id" => "np_7f3a9b2c", "amount" => 15_000.0)
      client = Class.new do
  ...
  it "fails safely when the profile does not declare terminal callback actions" do
    ...
    Dir.mktmpdir("provider-callback-capability") do |directory|
      ...
      signature = OpenSSL::HMAC.hexdigest(OpenSSL::Digest.new("SHA256"), "secret", body)
      expect(service.process_callback(raw_body: body, signature: signature)).to include("ok" => false, "error" => "callback action binding is unresolved")
    end
  end
  ...
  it "reuses an adapter-generated idempotency key for the same operation" do
    ...
    Dir.mktmpdir("provider-idempotency") do |directory|
      ...
      operation = { amount: 15_000.0, currency: "RUB", external_id: "op-retry", recipient: { type: "sbp", phone: "79001234567", bank_code: "044525225" } }
